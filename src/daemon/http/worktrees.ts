...
const suggest = z.object({ purpose: z.string().max(2000), model: z.string().trim().max(256).optional(), thinkingLevel: z.string().trim().max(256).optional(), worktreePrompt: z.string().max(8000).optional() }).strict();
const remove = z.object({ force: z.literal(true).optional(), deleteBranch: z.boolean().optional() }).strict();
...
export const createWorktreeRoutes = (service: WorktreeService, hooks?: { onRemoveWorkspace?: (workspaceId: string) => Promise<void> }, events?: WorkspaceEventHub): Hono => { const app = new Hono(); app.use("*", async (c, next) => { c.header("Cache-Control", "no-store"); return next(); });
  ...
  app.post("/api/projects/:projectId/worktrees/suggest", async (c) => { try { const x = suggest.parse(await readJsonBody(c.req.raw)); return ok(await service.suggest(id(c.req.param("projectId")), x.purpose, x.model, x.thinkingLevel, x.worktreePrompt ?? "")); } catch (e) { return error(e); } });
  ...
  app.post("/api/workspaces/:workspaceId/worktree/remove", async (c) => { try { const x = remove.parse(await readJsonBody(c.req.raw)); const workspaceId = id(c.req.param("workspaceId")); await hooks?.onRemoveWorkspace?.(workspaceId); const result = await service.remove(workspaceId, x.force === true, x.deleteBranch === true); events?.emitWorkspacesChanged({ reason: "remove", workspaceId }); return ok({ ok: true, branchDeleted: result.branchDeleted }); } catch (e) { return error(e); } }); return app; };
