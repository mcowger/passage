...
const error = (e: unknown) => Response.json({ error: e instanceof WorktreeError ? e.code : "invalid-request", ...(e instanceof WorktreeError ? { message: e.message } : {}) }, { status: e instanceof WorktreeError ? (e.code === "not-found" ? 404 : e.code === "force-required" ? 428 : 409) : 400, headers: { "Cache-Control": "no-store" } });
