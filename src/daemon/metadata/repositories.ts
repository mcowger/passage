...
export class ProjectRepository {
  ...
  save(value: Project): void { this.db.query("INSERT INTO projects (id, configured_root_path, canonical_root_path, display_label, icon_name, icon_color, use_project_icon, archived_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET configured_root_path=excluded.configured_root_path, canonical_root_path=excluded.canonical_root_path, display_label=excluded.display_label, icon_name=excluded.icon_name, icon_color=excluded.icon_color, use_project_icon=excluded.use_project_icon, archived_at=excluded.archived_at").run(value.id, value.configuredRootPath, value.canonicalRootPath, value.displayLabel, value.iconName ?? null, value.iconColor ?? null, integer(value.useProjectIcon ?? false), value.archivedAt); }
