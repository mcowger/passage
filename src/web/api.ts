...
export function createWorkspaceApi(
  ...
  return {
    ...
    async gitCommitAuto(id: string, input?: { model?: string; thinkingLevel?: string; commitPrompt?: string; agentId?: string }): Promise<{ head: string; message: string; status: GitStatus }> { return await request(`/api/workspaces/${encodeURIComponent(id)}/git/commit-auto`, { method: "POST", body: JSON.stringify(input ?? {}) }) as { head: string; message: string; status: GitStatus }; },
